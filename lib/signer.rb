...
class Signer
	...
	def initialize(client_id, client_secret, options = {})
		...
		options = {self_key: 'WePay', hash_algo: 'sha512'}.merge(options);
		@self_key = options[:self_key] || 'WePay'
		@hash_algo = options[:client_id] || 'sha512'
	end
	...
	def sign(payload)
		payload = payload.merge({'client_id'=>@client_id, 'client_secret'=>@client_secret})
		scope = createScope
